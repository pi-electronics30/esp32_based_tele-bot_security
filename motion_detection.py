
#include <WiFi.h>
#include <WiFiClientSecure.h>
#include "esp_camera.h"


// --- FILL YOUR DETAILS HERE---
const char* ssid = "";
const char* password = "";
const char* BOTtoken = "";
const char* CHAT_ID = "";


WiFiClientSecure clientTCP;

// Camera Pins (AI-Thinker/wrover module)
#define PWDN_GPIO_NUM     32
#define RESET_GPIO_NUM    -1
#define XCLK_GPIO_NUM      0
#define SIOD_GPIO_NUM     26
#define SIOC_GPIO_NUM     27
#define Y9_GPIO_NUM       35
#define Y8_GPIO_NUM       34
#define Y7_GPIO_NUM       39
#define Y6_GPIO_NUM       36
#define Y5_GPIO_NUM       21
#define Y4_GPIO_NUM       19
#define Y3_GPIO_NUM       18
#define Y2_GPIO_NUM        5
#define VSYNC_GPIO_NUM    25
#define HREF_GPIO_NUM     23
#define PCLK_GPIO_NUM     22

void setup() {
  Serial.begin(115200);
  
  WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
  Serial.println("\nWiFi Connected!");
  clientTCP.setInsecure(); 

  camera_config_t config;
  config.ledc_channel = LEDC_CHANNEL_0;
  config.ledc_timer = LEDC_TIMER_0;
  config.pin_d0 = Y2_GPIO_NUM;
  config.pin_d1 = Y3_GPIO_NUM;
  config.pin_d2 = Y4_GPIO_NUM;
  config.pin_d3 = Y5_GPIO_NUM;
  config.pin_d4 = Y6_GPIO_NUM;
  config.pin_d5 = Y7_GPIO_NUM;
  config.pin_d6 = Y8_GPIO_NUM;
  config.pin_d7 = Y9_GPIO_NUM;
  config.pin_xclk = XCLK_GPIO_NUM;
  config.pin_pclk = PCLK_GPIO_NUM;
  config.pin_vsync = VSYNC_GPIO_NUM;
  config.pin_href = HREF_GPIO_NUM;
  config.pin_sscb_sda = SIOD_GPIO_NUM;
  config.pin_sscb_scl = SIOC_GPIO_NUM;
  config.pin_pwdn = PWDN_GPIO_NUM;
  config.pin_reset = RESET_GPIO_NUM;
  config.xclk_freq_hz = 20000000;
  config.pixel_format = PIXFORMAT_JPEG;
  config.frame_size = FRAMESIZE_VGA;
  config.jpeg_quality = 12;
  config.fb_count = 2; // Isse 2 kar diya taaki ek overflow na ho

  esp_err_t err = esp_camera_init(&config);
  if (err != ESP_OK) { Serial.println("Camera init failed"); return; }
}

void captureAndSend() {
  // Purane buffers ko flush karne ke liye
  camera_fb_t * fb = NULL;
  
  // Trick: Do baar capture lo taaki latest frame mile aur overflow clear ho
  fb = esp_camera_fb_get();
  esp_camera_fb_return(fb); 
  fb = esp_camera_fb_get();
  
  if(!fb) {
    Serial.println("Camera capture failed, resetting...");
    ESP.restart(); // Agar capture fail ho toh board restart kardo
    return;
  }

  if (clientTCP.connect("api.telegram.org", 443)) {
    String boundary = "esp32cam";
    String head = "--" + boundary + "\r\nContent-Disposition: form-data; name=\"chat_id\"\r\n\r\n" + String(CHAT_ID) + "\r\n" +
                  "--" + boundary + "\r\nContent-Disposition: form-data; name=\"photo\"; filename=\"esp32cam.jpg\"\r\n" +
                  "Content-Type: image/jpeg\r\n\r\n";
    String tail = "\r\n--" + boundary + "--\r\n";

    clientTCP.println("POST /bot" + String(BOTtoken) + "/sendPhoto HTTP/1.1");
    clientTCP.println("Host: api.telegram.org");
    clientTCP.println("Content-Length: " + String(head.length() + fb->len + tail.length()));
    clientTCP.println("Content-Type: multipart/form-data; boundary=" + boundary);
    clientTCP.println();
    clientTCP.print(head);
    
    uint8_t *fbBuf = fb->buf;
    size_t fbLen = fb->len;
    for (size_t n=0; n<fbLen; n=n+1024) {
      if (n+1024 < fbLen) { clientTCP.write(fbBuf+n, 1024); }
      else { clientTCP.write(fbBuf+n, fbLen-n); }
    }
    clientTCP.print(tail);
    clientTCP.stop(); 
    Serial.println("Photo sent!");
  }

  esp_camera_fb_return(fb); 
}

void loop() {
  captureAndSend();
  delay(15000); // dealay of 15 sec is neccesary 
}
